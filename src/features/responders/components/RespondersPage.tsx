...
const RespondersPage: React.FC = () => {
  ...
  const currentResponders = filteredResponders.slice(indexOfFirstResponder, indexOfLastResponder);

  const handleNextPage = () => {
    if (currentPage < Math.ceil(filteredResponders.length / respondersPerPage)) {
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="flex items-center justify-center space-x-2 mt-20 text-sm text-gray-700">
        ...
        {Array.from({ length: Math.ceil(filteredResponders.length / respondersPerPage) }, (_, i) => (
          <button
            key={i + 1}
            type="button"
      ...
      {showCreateResponderModal && (
        <CreateResponderModal onClose={() => setShowCreateResponderModal(false)} />
      )}
    </div>
  );
};
