...
const ResponderLoginForm: React.FC = () => {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
                <label htmlFor="responderId" className="block text-sm font-medium text-[var(--foreground-main)] dark:text-[var(--ires-white)] mb-1">Responder ID</label>
                <input
                    id="responderId"
                    type="text"
                    {...register("responderId", { required: "Responder ID is required" })}
                ...
                />
                {errors.responderId && <span className="text-xs text-red-600 mt-1 block">{errors.responderId.message}</span>}
            </div>
            <div>
