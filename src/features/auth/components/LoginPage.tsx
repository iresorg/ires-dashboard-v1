...
const LoginPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="relative w-full h-full mx-auto flex flex-col md:flex-row rounded-2xl overflow-hidden">
        ...
        <div className="flex-1 flex flex-col justify-center px-8 py-12 md:py-16">
          ...
          <div>
            <div className="block dark:hidden">Light Only</div>
            <div className="hidden dark:block">Dark Only</div>
          </div>
          {/* Tab Switcher */}
          <div className="flex mb-8">
            {LOGIN_TYPES.map((type) => (
              <button
                key={type.value}
                className={`flex-1 px-4 py-2 mx-1 rounded-full font-semibold transition-colors duration-200 text-sm focus:outline-none ${loginType === type.value ? "bg-[var(--ires-red)] text-white" : "bg-gray-100 dark:bg-[var(--ires-black)] text-gray-700 dark:text-gray-200"}`}
        ...
        <div className="hidden md:flex flex-1 items-center justify-center bg-transparent p-8">
          <img src={securityLaptop} alt="Security Laptop" className="max-w-full h-auto" />
        </div>
